...
(* Pushes a vector on another structure provided a push function. *)
Equations vector_push {A U V : Type}
  (Useq : U -> list A)
  (Vseq : V -> list A)
  (push : forall (v : V) (u : U), { u' : U | Useq u' = Vseq v ++ Useq u })
  {n : nat} (v : vector V n) (u : U) :
    { u' : U | Useq u' = concat (map Vseq (vector_seq v)) ++ Useq u } :=
vector_push Useq Vseq p (V0) u0 := ? u0;
vector_push Useq Vseq p (V1 a1) u1 with p a1 u1 => { | ? u0 := ? u0 };
vector_push Useq Vseq p (V2 a1 a2) u2 with p a2 u2 => {
  | ? u1 with p a1 u1 => { | ? u0 := ? u0 }};
vector_push Useq Vseq p (V3 a1 a2 a3) u3 with p a3 u3 => {
  | ? u2 with p a2 u2 => { | ? u1 with p a1 u1 => { | ? u0 := ? u0 }}};
vector_push Useq Vseq p (V4 a1 a2 a3 a4) u4 with p a4 u4 => {
  | ? u3 with p a3 u3 => { | ? u2 with p a2 u2 => { | ? u1 with p a1 u1 => {
    | ? u0 := ? u0 }}}};
vector_push Useq Vseq p (V5 a1 a2 a3 a4 a5) u5 with p a5 u5 => {
  | ? u4 with p a4 u4 => { | ? u3 with p a3 u3 => { | ? u2 with p a2 u2 => {
    | ? u1 with p a1 u1 => { | ? u0 := ? u0 }}}}};
vector_push Useq Vseq p (V6 a1 a2 a3 a4 a5 a6) u6 with p a6 u6 => {
  | ? u5 with p a5 u5 => { | ? u4 with p a4 u4 => { | ? u3 with p a3 u3 => {
    | ? u2 with p a2 u2 => { | ? u1 with p a1 u1 => { | ? u0 := ? u0 }}}}}}.

(* Injects a vector on another structure provided an inject function. *)
Equations vector_inject {A U V : Type}
  (Useq : U -> list A)
  (Vseq : V -> list A)
  (inject : forall (u : U) (v : V), { u' : U | Useq u' = Useq u ++ Vseq v })
  (u : U) {n : nat} (v : vector V n) :
    { u' : U | Useq u' = Useq u ++ concat (map Vseq (vector_seq v)) } :=
vector_inject Useq Vseq i u0 (V0) := ? u0;
vector_inject Useq Vseq i u1 (V1 a1) with i u1 a1 => { | ? u0 := ? u0 };
vector_inject Useq Vseq i u2 (V2 a2 a1) with i u2 a2 => {
  | ? u1 with i u1 a1 => { | ? u0 := ? u0 }};
vector_inject Useq Vseq i u3 (V3 a3 a2 a1) with i u3 a3 => {
  | ? u2 with i u2 a2 => { | ? u1 with i u1 a1 => { | ? u0 := ? u0 }}};
vector_inject Useq Vseq i u4 (V4 a4 a3 a2 a1) with i u4 a4 => {
  | ? u3 with i u3 a3 => { | ? u2 with i u2 a2 => { | ? u1 with i u1 a1 => {
    | ? u0 := ? u0 }}}};
vector_inject Useq Vseq i u5 (V5 a5 a4 a3 a2 a1) with i u5 a5 => {
  | ? u4 with i u4 a4 => { | ? u3 with i u3 a3 => { | ? u2 with i u2 a2 => {
    | ? u1 with i u1 a1 => { | ? u0 := ? u0 }}}}};
vector_inject Useq Vseq i u6 (V6 a6 a5 a4 a3 a2 a1) with i u6 a6 => {
  | ? u5 with i u5 a5 => { | ? u4 with i u4 a4 => { | ? u3 with i u3 a3 => {
    | ? u2 with i u2 a2 => { | ? u1 with i u1 a1 => { | ? u0 := ? u0 }}}}}}.
