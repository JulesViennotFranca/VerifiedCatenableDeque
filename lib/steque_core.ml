...
let make_yellorange
: type a c. (a, yellow) prefix -> (a pair, c) chain -> a suffix -> a semi_steque
= fun p child s ->
  match child with
...
= fun child s sst ->
  ...
  (* At this stage, [child1] is the new child and [sst2] is the new
     ...
     Now we must perform a last operation on [child1] if [sst2] is a not ending,
     and then we can simply return [child1] and the suffix of [sst2]. *)
...
= fun (S c1) sst2 ->
  match c1 with
  (* If our first semi-regular steque is only a deque, we look at the number of
    ...
    | Some (w, d) ->
      ...
      | Some (x, d) ->
        ...
        | Some (y, d) ->
          ...
          | Some (z, d) ->
            (* If we have four or more elements, [c1] can be considered a green
               prefix. *)
            ...
            (* If [sst2] is a triple, its prefix is pushed on its child and the
               final steque is simply (c1, child(sst2), suffix(sst2))
...
= function
  (* First, we look at the case where the child of the red chain is a deque. *)
  | Chain (R, Packet (P2 (a, b), Hole, s), Ending d) ->
    begin match Deque.pop d with
    (* If the deque is empty, we can simply push the two elements of the prefix
       on the suffix. *)
    ...
    | Some (Pair (p, stored), d) ->
        ...
        (* Then, to get the child of the final green steque, we concatenate the
           steque from the poped pair and the rest of the deque. *)
