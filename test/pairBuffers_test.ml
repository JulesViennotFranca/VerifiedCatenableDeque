...
open Lib.Color.GYOR
module D = Lib.PairBuffers.Package.Core
open D
...
let fold_left_to_list t =
  List.rev
    (Lib.PairBuffers.Package.fold_left
      (fun xs x -> x::xs)
      []
      ({ core = t; length = 0 }))
...
let test _ deq =
  ...
  assert (lst = Lib.PairBuffers.Package.to_list
