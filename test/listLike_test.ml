...
module Test (D : module type of Cadeque.Deque) = struct
  ...
  let () = test "nth & nth_opt" @@ fun () ->
    ...
    for i = 0 to List.length lst - 1 do
      ...
      let y = try D.nth deq i with Failure e -> print_endline e; 0 in
      assert (x = y) ;
...
let () = header "Deque"
module Test_Deque = Test (Cadeque.Deque)
let () = Printf.printf "\n%!"

let () = header "Cadeque"
module Test_Cadeque = Test (Cadeque)
let () = Printf.printf "\n%!"